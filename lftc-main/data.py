...
def load_ohsumed_single(local_directory: str) -> tuple:
    ...
    #         # directory_name : train.csv / test.csv
    #         # subdirectory_path : data\datasets\Ohsumed\training\train.csv / data\datasets\Ohsumed\test\test.csv
    ...
    #         class_num, text, label = row.split(delimiter)
    ...
    def process(filename: str) -> list:
        ...
        with open(filename, newline='', encoding='utf-8') as csvfile:
            ...
            for row in csv_reader:
                # print(f"Row: {row}")
                # Each line contains three elements：class_num, text, label
                if len(row) == 3:
                    class_num, text, label = row
                    processed_data.append((label, text))
                else:
    ...
    train_dir = os.path.join(local_directory, "train\\train.csv")
    test_dir = os.path.join(local_directory, "test\\test.csv")
...
def load_r8(data_directory: str, delimiter: str = "\t") -> tuple:
    ...
    def process(filename: str) -> list:
        ...
        for row in text_list:
            delimiter = ","
            # print(row.split(delimiter))
            # label, text = row.split(delimiter) # original
            class_num, text, label = row.split(delimiter)
            processed_data.append((label, text))
        return processed_data
...
def load_DBpedia(data_directory: str, delimiter: str = "\t") -> tuple:
    """
    ...
    def process(filename: str) -> list:
        processed_data = []
        text_list = open(filename, "r", encoding='utf-8').read().strip().split("\n")

        for row in text_list:
            delimiter = ","
            if len(row.split(delimiter)) == 3:
                label, title, text = row.split(delimiter)
                processed_data.append((label, text))
        return processed_data
...
def load_AG_NEWS(data_directory: str, delimiter: str = "\t") -> tuple:
    """
    ...
    def process(filename: str) -> list:
        ...
        for row in text_list:
            ...
            if len(row.split(delimiter)) == 3:
                label, title, text = row.split(delimiter)
                processed_data.append((label.strip('"'), text))
        return processed_data
...
def load_kinnews_kirnews(
    ...
    def process(dataset: Iterable) -> list:
        ...
        for pair in dataset:
            ...
            pairs.append((label, title + " " + str(content)))
        return pairs

    # ds = load_dataset(dataset_name, data_split)
    # Load CSV file from local folder
    if data_split == "kinnews_cleaned":
        data_folder = "data/datasets/kinnews/cleaned"
    else:
        data_folder = "data/datasets/kirnews/cleaned"
...
def load_swahili(local_directory: str) -> tuple:
    """
    ...
    def process(filename: str) -> list:
        ...
        with open(filename, newline='', encoding='utf-8') as csvfile:
            ...
            for row in csv_reader:
                # print(f"Row: {row}")
                # Assuming each row contains three elements: class_num, text, label
                if len(row) == 2:
                    text, label = row
                    processed_data.append((label, text))
                else:
    ...
    train_dir = os.path.join(local_directory, "train.csv")
    test_dir = os.path.join(local_directory, "test.csv")
    train_ds, test_ds = process(train_dir), process(test_dir)

    return train_ds, test_ds

# Temporary use of some content in load_SogouNews
def load_SogouNews(local_directory: str) -> tuple:
    """
    Loads the Swahili dataset
    # Local loading
    Returns:
        ...
        with open(filename, newline='', encoding='utf-8') as csvfile:
            ...
            for row in csv_reader:
                if len(row) == 3:
                    ...
                    # if len(text) < 15000 and label != "label":
                    #     processed_data.append((label, text))
                    if label != "label":
                        processed_data.append((label, text))
                    else:
